          ...
          if(!$execute || $attachmentData['type'] == 'buysell') {
            ...
            if($attachmentData['type'] == 'buysell') {
              if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('ealbum'))
                $plugin =  Engine_Api::_()->loadClass('Ealbum_Plugin_Composer');
      ...
      if ($feeling_type == 1) {
        ...
        if (Engine_Api::_()->hasItemType($itemType)) {
          ...
          if($item) {
            ...
            $html .= '<li data-type="'.$itemType.'" data-icon="'.Engine_Api::_()->storage()->get($feelingsIcon->file_id, "")->getPhotoUrl().'" data-title="'.$item->getTitle().'" class="'.$liClassName.' clearfix" data-rel='.$result->id.'><a href="javascript:void(0);" class="activity_feelingactivitytypea">'.$photo_icon_photo.'<span>'.$item->getTitle().'</span></a></li>';
          }
        }
      }
    }
