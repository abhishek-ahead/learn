...
class Activity_Widget_FeedController extends Engine_Content_Widget_Abstract
{
  ...
  public function indexAction() {
    ...
    if ($widgetIds) {
      $params = Engine_Api::_()->ecommunityads()->getWidgetParams($widgetIds);
    ...
    if (!$isMember) {
      if ($actionTypeGroup && isset($groupedActionTypes[$actionTypeGroup])) {
        ...
        if (Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('album') &&  ($actionTypeGroup == 'ealbum' || $actionTypeGroup == 'album'))
          $actionTypeFilters = array_merge($actionTypeFilters, $groupedActionTypes['photo']);
        else if (Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('evideo') && $actionTypeGroup == 'evideo')
      ...
      if (!empty($actions) && engine_count($actions) > 0) {
        foreach ($actions as $action) {
          ...
          if($action->getObject()) {
            ...
            if(!$viewPermission && $object->getType() != 'activity_buysell' && $object->getType() != 'activity_file') {
              continue;
            }            
