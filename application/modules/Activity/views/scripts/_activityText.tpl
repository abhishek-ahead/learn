...
if (!$this->getUpdate && ($this->ulInclude)):
  ...
  <div class="activity_feed  clearfix">
    ...
    if (defined('ECOMMUNITYADS')) {
      $communityAdsEnable = $settings->getSetting('ecommunityads_advertisement_enable', 1);
      $communityAdsDisplay = $settings->getSetting('ecommunityads_advertisement_display', 3);
      $communityAdsDisplayFeed = $settings->getSetting('ecommunityads_advertisement_displayfeed', 1);
      if (!$this->isMemberHomePage && !$communityAdsDisplayFeed)
        $communityAdsEnable = 0;
      $communityAdsDisplayAds = $settings->getSetting('ecommunityads_advertisement_displayads', 5);
    }
    ...
    foreach ($actions as $action): //(goes to the end of the file)
      ...
      if (@$communityAdsEnable && ($contentCount && $contentCount % $communityAdsDisplayAds == 0)) { ?>
        <li class="clearfix activity_community_ads activity_pinfeed_hidden _photo<?php echo $this->userphotoalign; ?>">
          ...
          include ('application/modules/Ecommunityads/views/scripts/_activityAds.tpl');
          ?>
        </li>
        <?php
      }
