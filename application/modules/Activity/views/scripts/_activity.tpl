...
<?php if (!$this->noList): ?>
  <li id="activity-item-<?php echo $action->action_id ?>" data-activity-feed-item="<?php echo $action->action_id ?>"
    class="activity_pinfeed activity_pinfeed_hidden clearfix _photo<?php echo $this->userphotoalign; ?><?php if (!empty($fromActivityFeed)) { ?> ecommunityads_ad_id ecmads_ads_listing_item<?php } ?>"
    <?php if (!empty($fromActivityFeed)) { ?> rel="<?php echo $ad->getIdentity(); ?>" <?php } ?>>
  <?php endif; ?>

  <?php if (!empty($fromActivityFeed)) { ?>
    <?php include ('application/modules/Ecommunityads/views/scripts/widget-data/_hiddenData.php'); ?>
  <?php } ?>
  <section class="block <?php if (!empty($fromActivityFeed)) { ?> ecmads_ads_item_img <?php } ?>">
          ...
          <?php if (!empty($fromActivityFeed) && empty($hideOptions) && $ad->user_id != $this->viewer()->getIdentity()) { ?>
            <div class="activity_feed_options options_menu dropdown">
              ...
              <ul class="dropdown-menu options_menu dropdown-menu-end">
                ...
                <!--<li><a target="_blank" href="<?php echo $this->url(array('module' => 'ecommunityads', 'controller' => 'index', 'action' => 'why-seeing'), 'ecommunityads_whyseeing', false); ?>" class="ecomm_seeing_ad"><?php echo $this->translate('why am i seeing this?'); ?></a></li>-->
                <li>
    ...
    <div class='feed_item_body clearfix'>
      ...
      <?php endif; ?>
      <?php if ($buysellActive && empty($isBuySellShareFeed)) { ?>
          ...
          <div class="activity_feed_item_buysell">
            ...
            <?php } else if($buysell->location) { ?>
              <div class="activity_feed_item_buysell_location font_color_light">
                <i class="icon_map"></i>
                <span>
                  <a href="<?php echo 'http://maps.google.com/?q='.$buysell->location; ?>" target='_blank' class="font_color_light"><?php echo $buysell->location; ?></a>
                </span>
              </div>
            <?php } ?>
    ...
    <?php $ecommunityads = empty($ecommunityads) ? engine_in_array('ecommunityads', $this->enabledModuleNames) : $ecommunityads; ?>
    <?php if (empty($fromActivityFeed) && empty($_SESSION['fromActivityFeed']) && !empty($action) && !$action->schedule_time && (empty($this->filterFeed) || $this->filterFeed != 'hiddenpost') && empty($previousAction) && $ecommunityads && is_array(Engine_Api::_()->ecommunityads()->allowedTypes($action)) && engine_in_array('boos_post', Engine_Api::_()->ecommunityads()->allowedTypes($action)) && Engine_Api::_()->ecommunityads()->getAllowedActivityType($action->type) && ($action->subject_id == $this->viewer()->getIdentity() && $action->subject_type == "user")) {
      if (Engine_Api::_()->authorization()->isAllowed('ecommunityads', $this->viewer(), 'create')) {
        ?>
        <div class="activity_boost_post clearfix clear">
          <?php if (!empty($action->view_count)) { ?>
            <div class="activity_boost_post_reach clearfix  font_color_light">
              <?php echo $this->translate("%s people Reached", $action->view_count); ?>
            </div>
          <?php } ?>
          <div class="clearfix activity_boost_btn floatR">
            <a
              href="<?php echo $this->url(array("controller" => "index", "action" => "create", 'action_id' => $action->action_id), 'ecommunityads_general', true); ?>"><?php echo $this->translate('Boost Post'); ?></a>
          </div>
        </div>
        <?php
      }
