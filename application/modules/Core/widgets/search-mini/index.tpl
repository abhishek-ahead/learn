...
<div id='global_search_form_container' class="core_search_form">
  ...
  <div class="core_search_form_wrap">
    ...
    <div id="recent_search_data" class="header_search_recent notifications_donotclose" style="display:none;">
      <?php if($this->viewer()->getIdentity()) { ?>
        <div id="user_search_query_all">
          <div class="header_search_recent_head" id="header_search_recent_head" <?php if(engine_count($this->getResults) == 0) { ?> style="display:none;" <?php } ?> >
          ...
          <ul class="header_search_recent_list" id="header_search_recent_list">
            <?php foreach($this->getResults as $result) { ?>
              <li id="search_query_<?php echo $result->recentsearch_id; ?>" >
              <a href="<?php echo $this->url(array('controller' => 'search'), 'default', true). '?query='.$result->query.'&type='.$result->type; ?>" class="header_search_recent_list_item">
                  <div class="_thumb">
                    <?php if($result->id && $result->type) { ?>
                      <?php $item = Engine_Api::_()->getItem(strtolower($result->type), $result->id); ?>
      ...
      <?php if(engine_count($this->hashtags) > 0) { ?>
        <div class="header_search_recent_head">
          <span><?php echo $this->translate("Trending Hashtags");?></span>
        </div>
        <ul class="header_search_recent_list">
...
<script type="application/javascript">
  en4.core.runonce.add(function() {
    ...
    scriptJquery(".search_mobile_btn").click(function(){
      scriptJquery(".core_search_form_wrap").toggleClass("search_mobile_btn_active");
    });
    ...
    AutocompleterRequestJSON('global_search_field', "<?php echo $this->url(array('module' => 'core', 'controller' => 'search', 'action' => 'global-search'), 'default', true) ?>", function(selecteditem) {
      ...
      var searchDataResource = '<li id="search_query_'+randomId+'"><a href="search/index/query/'+selecteditem.value+'/type/'+selecteditem.resource_type+'" class="header_search_recent_list_item"><div class="_thumb">'+selecteditem.photo+'</div><div class="_info"><p class="m-0 _title">'+selecteditem.value+'</p></div></a><a href="'+selecteditem.url+'" class="user_recent_search_remove _clear link_inherit center_item rounded-circle" data-id="'+randomId+'" data-query="'+selecteditem.value+'"><i class="icon_cross"></i></a></li>';
      scriptJquery('#header_search_recent_list').append(searchDataResource);
