...
<script type="text/javascript">
  var formCheck = false;
  scriptJquery(document).on('submit', '#user_form_auth_forgot', function(e) {
    if(!formCheck) {
      e.preventDefault();
  ...
  <?php //if( $settings->getSetting('user.signup.enabletwostep', 0) == 1 || !empty($otpsms_signup_phonenumber)) { ?>
      ...
      (scriptJquery.ajax({
        ...
        success : function(response) {
          if(response.status) {
            ...
            scriptJquery('#user_signup_email').modal('show').show
            document.getElementById('send_signup_form').style.display = 'block';
    ...
    function validateTwoStepCode() {
      ...
      if(code === '') {
        scriptJquery('#error_message').show().html("Please enter the valid code.");
        return;
      }
