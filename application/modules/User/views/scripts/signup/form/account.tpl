  ...
  <?php if( $settings->getSetting('user.signup.enabletwostep', 0) == 1 || !empty($otpsms_signup_phonenumber)) { ?>
    ...
    AttachEventListerSE('blur', '#email', function(e) {
      ...
      if(scriptJquery('#verifed_text').length == 0) {
        ...
        if(scriptJquery('#email').val()) {
          verifyPhoneNumber(scriptJquery('#email').val(), scriptJquery('#country_code').val());
        }
      }
    });
    ...
    function verifyPhoneNumber(phone_number, country_code) {
      ...
      if(scriptJquery('#phone_number_loading').length == 0) {
        scriptJquery('#email-element').append('<a href="javascript:;" class="verify_email verify_email_icon" id="phone_number_loading"><i class="fas fa-spinner fa-spin"></i></a>');
      }
        ...
        success : function(response) {
          ...
          if(response.status) {
            if(scriptJquery('#verify_email').length == 0) {
              scriptJquery('#email-element').append('<a href="javascript:;" data-bs-toggle="modal" data-bs-target="#user_signup_email" class="verify_email verify_email_icon" id="verify_email"><?php echo $this->string()->escapeJavascript($this->translate('Verify')); ?></a>');
            }
          } else {
            scriptJquery("#verify_email").remove();
            scriptJquery('#email-element').after('<div class="error font_small mt-2" id="phone_number_exists">'+response.message+'</div>');
          }
        }
      }));
    }
    ...
    function validateTwoStepCode() {
      ...
      if(code === '') {
        scriptJquery('#error_message').show().html("<?php echo $this->string()->escapeJavascript($this->translate('Please enter the valid code.')); ?>");
        return;
      }
