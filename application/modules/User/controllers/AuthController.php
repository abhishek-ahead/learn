...
//                 return $this->_redirect($url, array('prependBase' => false));
  ...
  function verifyLoginAction() {
    ...
    echo json_encode(array('error'=>0,'url'=>$this->view->url(array('module'=>'user','controller'=>'auth', 'action'=>'login'),'default',true)));die;
  }
  ...
  function verifyAction() {
    ...
    $form->setAction($this->view->url(array('module'=>'user','controller'=>'auth','action'=>'verify-login'),'default',true));
  ...
  public function quickloginAction() {
    ...
    if(Engine_Api::_()->user()->checkCredential($id, $password, $user) && $user->getIdentity() == $id) {
      ...
      // Redirect
      if (null === $this->_helper->contextSwitch->getCurrentContext()) {
        echo json_encode(array('status' => false, 'redirect_url' => $this->_helper->redirector->gotoRoute(array(), 'default', true)));die;
        return;
      } else {
        echo json_encode(array('status' => true, 'redirect_url' => $this->view->url(array(), 'default', true)));die;
        return;
      }
    }
    echo json_encode(array('status' => false, 'message' => $this->view->translate('Your password is not correct.')));die;
